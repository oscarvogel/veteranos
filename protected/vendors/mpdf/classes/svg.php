	...
	function svgStyle($critere_style, $attribs, $element){
		...
		// mPDF 4.4.005   Used as indirect setting for currentColor
			...
			if ($col) {
				// mPDF 5.0.051
				// mPDF 5.3.74
				if ($col[$2]==5) {	$critere_style['fill-opacity'] = ord($col[$2]/100); }	// RGBa
				if ($col[$2]==6) {	$critere_style['fill-opacity'] = ord($col[$2]/100); }	// CMYKa
...
*/
		...
		// mPDF 4.4.005   Used as indirect setting for currentColor
			...
			if ($col) {
				// mPDF 5.0.051
				// mPDF 5.3.74
				if ($col[$2]==5) {	$critere_style['stroke-opacity'] = ord($col[$2]/100); }	// RGBa
				if ($col[$2]==6) {	$critere_style['stroke-opacity'] = ord($col[$2]/100); }	// CMYKa
		...
		if(!function_exists("xml_svg2pdf_start")){	// mPDF 5.3.76
			...
			function xml_svg2pdf_start($parser, $name, $attribs){
				...
				case 'stop':
						...
						// mPDF 5.0.051
						// mPDF 5.3.74
						if ($col[$2]==3 || $col[$2]==5) {	// RGB
							$color_final = sprintf('%.3F %.3F %.3F',ord($col[$2])/255,ord($col[$2])/255,ord($col[$2])/255);
							$svg_class->svg_gradient[$last_gradid]['colorspace']='RGB';
						}
						else if ($col[$2]==4 || $col[$2]==6) {	// CMYK
							$color_final = sprintf('%.3F %.3F %.3F %.3F',ord($col[$2])/100,ord($col[$2])/100,ord($col[$2])/100,ord($col[$2])/100);
							$svg_class->svg_gradient[$last_gradid]['colorspace']='CMYK';
						}
						else if ($col[$2]==1) {	// Grayscale
							$color_final = sprintf('%.3F',ord($col[$2])/255);
						...
						// mPDF 5.0.051
						// mPDF 5.3.74
						else if ($col[$2]==5) {	// RGBa
							$stop_opacity = ord($col[$2]/100);
						}
						else if ($col[$2]==6) {	// CMYKa
							$stop_opacity = ord($col[$2]/100);
						}
