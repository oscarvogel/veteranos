...
class PDFBarcode {
	...
	protected function barcode_c128($code, $type='B', $ean=false) {
		...
		switch(strtoupper($type)) {
			...
			case 'C': {
				...
				$hclen = (strlen($code) / 2);
				for ($i = 0; $i < $hclen; ++$i) {
					$new_code .= chr(intval($code[$1].$code[$1]));
				}
				$code = $new_code;
				break;
			}
	...
	protected function barcode_eanext($code, $len=5) {
		...
		if ($len == 2) {
			...
			$r = (3 * ($code[$2] + $code[$2] + $code[$2])) + (9 * ($code[$2] + $code[$2]));
			$r %= 10;
		} else {
			return false;
		}
	...
	protected function barcode_imb($code) {
		...
		$binary_code = bcadd($binary_code, $tracking_number[$2]);
