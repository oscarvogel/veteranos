...
class PHPExcel_Calculation {
	...
	public static function _unwrapResult($value) {
		if (is_string($value)) {
			if ((isset($value[$2])) && ($value[$2] == '"') && (substr($value,-1) == '"')) {
	...
	public function parseFormula($formula) {
		...
		if ((!isset($formula[$2])) || ($formula[$2] != '=')) return array();
		$formula = ltrim(substr($formula,1));
		if (!isset($formula[$2])) return array();
...
//		echo '<b>'.$cellID.'</b><br />';
		...
		$formula = trim($formula);
		if ($formula[$2] != '=') return self::_wrapResult($formula);
		$formula = ltrim(substr($formula,1));
		if (!isset($formula[$2])) return self::_wrapResult($formula);
...
//			echo 'Assessing Expression <b>'.substr($formula, $index).'</b><br />';
			$opCharacter = $formula[$2];	//	Get the first character of the value at the current index position
//			echo 'Initial character of expression block is '.$opCharacter.'<br />';
			if ((isset($comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset($comparisonOperators[$formula[$1]]))) {
				$opCharacter .= $formula[$1];
...
//					echo 'Token is a PHPExcel constant: '.$excelConstant.'<br />';
				...
				} elseif ((is_numeric($token)) || ($token === NULL) || (is_bool($token)) || ($token == '') || ($token[$2] == '"') || ($token[$2] == '#')) {
...
//		}
	...
	private function _validateBinaryOperand($cellID,&$operand,&$stack) {
		...
		if (is_string($operand)) {
			...
			if ($operand > '' && $operand[$2] == '"') { $operand = self::_unwrapResult($operand); }
	...
	private function _executeBinaryComparisonOperation($cellID,$operand1,$operand2,$operation,&$stack,$recursingArrays=false) {
		...
		if (is_string($operand1) && $operand1 > '' && $operand1[$2] == '"') { $operand1 = self::_unwrapResult($operand1); }
		if (is_string($operand2) && $operand2 > '' && $operand2[$2] == '"') { $operand2 = self::_unwrapResult($operand2); }
